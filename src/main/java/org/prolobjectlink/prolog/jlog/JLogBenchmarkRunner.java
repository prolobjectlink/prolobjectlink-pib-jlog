 ...
 */
public class JLogBenchmarkRunner {
    ...
    public static class ExecutionPlan {
        ...
        public void setUp() {
            ...
            PrologProvider provider = Prolog.getProvider(JLog.class);
...
//              20 * D1 < 21 * D2.
            ...
            engine.asserta(provider.newAtom("query"), /* :- */
                    provider.newStructure("density", c1, d1),
                    provider.newStructure("density", c2, d2),
                    provider.newStructure(d1, ">", d2),
                    provider.newStructure(left, "<", rigth)
            );
...
//                  D is ( P * 100 ) / A.
            ...
            engine.asserta(provider.newStructure("density", c, d), /* :- */
                    provider.newStructure("pop", c, p),
                    provider.newStructure("area", c, a),
                    provider.newStructure(d, "is", provider.newStructure(provider.newStructure(p, "*", provider.newInteger(100)), "/", a))
            );
    ...
    public void benchQuery(ExecutionPlan plan) {
        PrologTerm[] oneSolution = plan.engine.query("query").oneSolution();
    }
    ...
    public void benchQueryAll(ExecutionPlan plan) {
        PrologTerm[][] allSolutions = plan.engine.query("employee(Name,Dpto,Scale),department(Dpto,DepartmentName),salary(Scale,Money)").allSolutions();
    }

    public static void main(String[] args) throws RunnerException, IOException {
        DecimalFormat df = new DecimalFormat("0.000");
        double referenceScore = 0.155;
        double maxDeviation = 0.05;
        OptionsBuilder builder = new OptionsBuilder();
        builder.include(JLogBenchmarkRunner.class.getSimpleName());
